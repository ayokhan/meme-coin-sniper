...
export type NovaPulseSubTab = "futures" | "forex";
...
export default function NovaPulsePanel({
  ...
  useEffect(() => {
    const allowed =
      (localTab === "futures" && novaScalpAgent) || (localTab === "forex" && novaForexScalpAgent);
    if (allowed) return;
    const fallback: NovaPulseSubTab | null = novaScalpAgent ? "futures" : novaForexScalpAgent ? "forex" : null;
    if (fallback) {
      setLocalTab(fallback);
      onSubTabChange(fallback);
    }
  }, [localTab, novaScalpAgent, novaForexScalpAgent, onSubTabChange]);
  ...
  if (!novaScalpAgent && !novaForexScalpAgent) {
    return (
    ...
    <div className="space-y-4">
      <div className="rounded-lg border border-zinc-200 dark:border-zinc-700 p-4">
        ...
        <p className="text-xs text-muted-foreground">
          ...
          guaranteed profits. Size trades first in the{" "}
          <a href="/?tab=pnl-calculator" className="text-cyan-600 dark:text-cyan-400 hover:underline">
            PnL Calculator
          </a>{" "}
          tab before you send them to the scalper or NovaQ.
        </p>
      </div>
