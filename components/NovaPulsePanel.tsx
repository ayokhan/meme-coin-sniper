...
export default function NovaPulsePanel({
  ...
  useEffect(() => {
    if (localTab === "futures" && !novaScalpAgent && novaForexScalpAgent) {
      setLocalTab("forex");
      onSubTabChange("forex");
    } else if (localTab === "forex" && !novaForexScalpAgent && novaScalpAgent) {
      setLocalTab("futures");
      onSubTabChange("futures");
    }
    ...
    <div className="space-y-4">
      <div className="rounded-lg border border-zinc-200 dark:border-zinc-700 p-4">
        ...
        <p className="text-xs text-muted-foreground">
          ...
          guaranteed profits. Futures uses Nova Scalp Agent; Forex uses Nova Forex Agent.
        </p>
      </div>
