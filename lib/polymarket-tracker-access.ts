...
export async function getPolymarketTrackerAccess(session: Session | null): Promise<PolymarketTrackerAccessResult> {
  ...
  if (tier !== "vip") {
    return { ok: false, status: 403, error: "VIP with Nova Polymarket Bot access required." };
  }
  const user = await prisma.user.findUnique({ where: { id: userId } });
  const polyOn = !!(user as { polymarketBotOnDemand?: boolean } | null)?.polymarketBotOnDemand;
  if (!polyOn) {
    return { ok: false, status: 403, error: "Ask admin to enable Nova Polymarket Bot (on demand) for your account." };
  }
