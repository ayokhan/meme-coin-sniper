...
function RegisterForm() {
  ...
  return (
    <div
      ref={topRef}
      className="min-h-screen flex items-start sm:items-center justify-center bg-zinc-100 dark:bg-zinc-950 px-3 sm:px-4 py-6 sm:py-10"
    >
      <Card className="w-full max-w-md border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-lg mt-2">
            {success ? "You're registered" : "Create a free account"}
          </CardTitle>
          {!success && (
            <p className="text-sm text-muted-foreground">
              Sign up for free — no credit card required. Save watchlists, track wallets, and upgrade to VIP when you
              are ready.
            </p>
          )}
        </CardHeader>
        <CardContent className="space-y-4">
          {sitePromo?.active && sitePromo.showOnRegister && !success && (
          ...
          {success ? (
            <div className="rounded-xl border border-emerald-500/30 bg-emerald-50 dark:bg-emerald-950/40 px-4 py-5 text-center space-y-3">
              <CheckCircle2 className="h-10 w-10 mx-auto text-emerald-600 dark:text-emerald-400" aria-hidden />
              <p className="text-base font-semibold text-emerald-900 dark:text-emerald-100">Registration complete</p>
              <p className="text-sm text-emerald-800/90 dark:text-emerald-200/90 leading-relaxed">{success}</p>
              <Button
                asChild
                ...
                <form onSubmit={handleRegister} className="space-y-3">
                  ...
                  <label className="flex items-center gap-2 text-sm text-zinc-700 dark:text-zinc-300 cursor-pointer">
                    <input
                      ...
                      className="rounded border-zinc-300 dark:border-zinc-600"
                    />
                    Join <span className="font-semibold">NovaConnect</span> (social portal — show my profile as online
                    to other traders)
                  </label>
                  <p className="text-xs text-muted-foreground">
                  ...
                  <Button
                    type="submit"
                    className="w-full"
                    disabled={loading || !acceptCommunityRules || !acceptPresencePrivacy}
                  >
