...
export async function GET(request: Request) {
  try {
    ...
    const dexAllow = GO_HUNTING_DEX_ALLOWLIST.hyperevm[view];
    ...
    const pairs = await getMemeRunnerChainPairs({
      ...
      searchQueries: ["hyperevm", "HYPE", "meme", "new token", "hyperswap"],
      maxResults: 350,
    });

    const filtered = filterPairsForGoHuntingView(pairs, view, "hyperevm");
    ...
    return NextResponse.json({
      ...
      maxAgeMinutes: effectiveMaxAge,
      view,
      description: `HyperEVM Go Hunting · ${viewLabel}: last ${effectiveMaxAge}m (AI viral score on each).`,
    });
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : "HyperEVM new pairs failed";
