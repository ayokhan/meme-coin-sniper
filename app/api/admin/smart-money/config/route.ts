...
export async function GET() {
  ...
  const cfgDb = (prisma as unknown as PrismaExt).smartMoneyConfig;
  const ulDb = (prisma as unknown as PrismaExt).smartMoneyUserLimit;
...
export async function PATCH(request: Request) {
  ...
  if (typeof body.buyAlertUsd === "number") update.buyAlertUsd = Math.max(0, body.buyAlertUsd);
  if (typeof body.bigBuyAlertUsd === "number") update.bigBuyAlertUsd = Math.max(0, body.bigBuyAlertUsd);
  if (typeof body.holdAlertMinutes === "number") update.holdAlertMinutes = Math.max(1, Math.round(body.holdAlertMinutes));
  if (typeof body.maxWallets === "number") update.maxWallets = Math.max(1, Math.min(50, Math.round(body.maxWallets)));
