...
export async function GET(request: Request) {
  try {
    ...
    const dexAllow = GO_HUNTING_DEX_ALLOWLIST.bsc[view];
    ...
    const pairs = await getMemeRunnerChainPairs({
      ...
      searchQueries:
        view === "final_stretch"
          ? ["four.meme", "fourmeme", "bsc meme"]
          : ["pancakeswap", "bsc meme", "new token"],
      maxResults: 350,
    });

    const filtered = filterPairsForGoHuntingView(pairs, view, "bsc");
