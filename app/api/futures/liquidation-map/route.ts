...
export async function POST(request: Request) {
  try {
    ...
    const exchangeLiqRaw = Number(body.exchangeLiquidationPrice);
    const exchangeLiquidationPrice =
      Number.isFinite(exchangeLiqRaw) && exchangeLiqRaw > 0 ? exchangeLiqRaw : null;
    if (!symbol) {
    ...
    const tradeCheck = hasTradePlan
      ? buildLiquidationMapTradeCheck({
          traderType,
          entry,
          exit,
          leverage,
          exchangeLiquidationPrice,
          markPrice,
          bias: analysis.bias,
          trend,
          marketStructure,
